...
<div class="box" style="display:flex, justify-content:center; align-items:center;">
	<div style="flex:1">
		<a href="{{ route('addAlert') }}">Signaler</a>
		<a href="">S.O.S</a>
	</div>
		...
		<ul style="" id="">
			@if(count($incidents))
				@foreach($incidents as $incident)
			    	<li>
			            <span></span>
			            <div class="title">{{ $incident->category->label }}</div>
			            <div class="info">@if($incident->valid <= 3) Probable @else Attention @endif: A proximité de : {{ $incident->street }}, {{ $incident->city }}</div>
			            <div class="time">
			...
			@else
				<li>
			       	<span></span>
			        <div class="title">Rien de signalé !</div>
			        <div class="info">Mais restez vigilents èé !</div>
			       	</li>
			@endif
		</ul>
	</div>
